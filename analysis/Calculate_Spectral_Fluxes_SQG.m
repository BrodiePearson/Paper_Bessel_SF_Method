%% Code to diagnose SQG spectral fluxes from GeophysicalFlows.jl
...
for nn=1:file_no
    ...
    dbdx = real(ifft2(ifftshift(1i*k_mat'.*fftshift(fft2(b)))));
    dbdy = real(ifft2(ifftshift(1i*l_mat'.*fftshift(fft2(b)))));
    ...
    dbdx_fd=(circshift(b,-1,1)-circshift(b,1,1))./(2*x_sep);
    dbdy_fd=(circshift(b,-1,2)-circshift(b,1,2))./(2*y_sep);
    ...
    for kk=1:size(tmp_kr,1)
        tmp_bb_Flux(kk) = -2*sum(bb_flux_div(k_mat.^2 + ...
            l_mat.^2 >= tmp_kr(kk)^2)*dk*dl)/((2*pi)^2);
        tmp_KE_Flux(kk) = -sum(KE_flux_div(k_mat.^2 + ...
            l_mat.^2 >= tmp_kr(kk)^2)*dk*dl)/((2*pi)^2);

        tmp_bb_Flux_fd(kk) = -2*sum(bb_flux_div_fd(k_mat.^2 + ...
            l_mat.^2 >= tmp_kr(kk)^2)*dk*dl)/((2*pi)^2);
        tmp_KE_Flux_fd(kk) = -sum(KE_flux_div_fd(k_mat.^2 + ...
            l_mat.^2 >= tmp_kr(kk)^2)*dk*dl)/((2*pi)^2);

        %tmp_bb_spectrum = bb_spectrabb_spectra = fftshift(real(conj(b_f).*b_f)); = fftshift(real(conj(b_f).*b_f));
    end
    
    %% Calculate Spectral Fluxes from data
    
    % Shift the spectra so zero at lowest wavenumber/largest scale 
    bb_Flux(:,nn) = tmp_bb_Flux ;%- tmp_bb_Flux(1);
    KE_Flux(:,nn) = tmp_KE_Flux ;%- tmp_KE_Flux(1);

    bb_Flux_fd(:,nn) = tmp_bb_Flux_fd ;%- tmp_bb_Flux(1);
    KE_Flux_fd(:,nn) = tmp_KE_Flux_fd ;%- tmp_KE_Flux(1);
    bb_Flux_fd_flip(:,nn) = tmp_bb_Flux_fd - tmp_bb_Flux_fd(1);
    KE_Flux_fd_flip(:,nn) = tmp_KE_Flux_fd - tmp_KE_Flux_fd(1);

    kr(:,nn) = tmp_kr;
    kinetic_energy(nn,1) = KE;
    %bb_spectrum(:,nn) = bb_spectra; %- tmp_bb_Flux(1);
...
Spectral_Flux.KE_Flux = mean(KE_Flux,2, 'omitnan');
Spectral_Flux.bb_Flux = mean(bb_Flux,2, 'omitnan');

Spectral_Flux.KE_Flux_fd = mean(KE_Flux_fd,2, 'omitnan');
Spectral_Flux.bb_Flux_fd = mean(bb_Flux_fd,2, 'omitnan');
Spectral_Flux.KE_Flux_fd_flip = mean(KE_Flux_fd_flip,2, 'omitnan');
Spectral_Flux.bb_Flux_fd_flip = mean(bb_Flux_fd_flip,2, 'omitnan');


Spectral_Flux.KE_Flux_snapshots = KE_Flux;
Spectral_Flux.bb_Flux_snapshots = bb_Flux;

Spectral_Flux.Wavenumber = kr(:,1);

%% Save structure functions ERRORS across snapshots (standard deviation)
Spectral_Flux.KE_Flux_std = std(KE_Flux,0,2, 'omitnan');
Spectral_Flux.bb_Flux_std = std(bb_Flux,0,2, 'omitnan');


%% Save PE and KE diagnostics to structure

Spectral_Flux.bb_Work_Rate = bb_Work_Rate;
Spectral_Flux.bb_Drag_Rate = bb_Drag_Rate;
Spectral_Flux.bb_Diss_Rate = bb_Diss_Rate;
...
figure
subplot(1,2,1)
semilogx(Spectral_Flux.Wavenumber,Spectral_Flux.bb_Flux,'k-')
hold on
semilogx(Spectral_Flux.Wavenumber, ...
    Spectral_Flux.bb_Flux+Spectral_Flux.bb_Flux_std,'k:')
semilogx(Spectral_Flux.Wavenumber, ...
    Spectral_Flux.bb_Flux-Spectral_Flux.bb_Flux_std,'k:')
semilogx([7 max(Spectral_Flux.Wavenumber)], ...
    Spectral_Flux.bb_Diss_Rate*[1 1], 'r--')
semilogx([7 max(Spectral_Flux.Wavenumber)], ...
    mean(Spectral_Flux.bb_Diss_Rate, 'omitnan')*[1 1], 'r-','LineWidth',4)
semilogx([min(Spectral_Flux.Wavenumber) 7], ...
    -Spectral_Flux.bb_Drag_Rate*[1 1], 'r--')
semilogx([7 max(Spectral_Flux.Wavenumber)], ...
    5e-6*[1 1], 'b-','LineWidth',4)
semilogx([7 max(Spectral_Flux.Wavenumber)], ...
    (Spectral_Flux.bb_Diss_Rate + Spectral_Flux.bb_Drag_Rate)*[1 1], 'b--')
semilogx([7 7], ...
    [-Spectral_Flux.bb_Drag_Rate Spectral_Flux.bb_Diss_Rate], 'r--')
title('Flux of buoyancy variance')
xlim([min(Spectral_Flux.Wavenumber) max(Spectral_Flux.Wavenumber)])


subplot(1,2,2)
semilogx(Spectral_Flux.Wavenumber,Spectral_Flux.KE_Flux,'k-')
hold on
semilogx(Spectral_Flux.Wavenumber, ...
...
semilogx(Spectral_Flux.Wavenumber, ...
    Spectral_Flux.KE_Flux-Spectral_Flux.KE_Flux_std,'k:')
semilogx([7 max(Spectral_Flux.Wavenumber)], ...
    0.5*Spectral_Flux.bb_Diss_Rate*[1 1], 'r--')
semilogx([min(Spectral_Flux.Wavenumber) 14], ...
    -0.5*Spectral_Flux.bb_Drag_Rate*[1 1], 'r--')
semilogx([7 max(Spectral_Flux.Wavenumber)], ...
    5e-6*[1 1], 'b-','LineWidth',4)
...
figure
subplot(1,2,1)
semilogx(Spectral_Flux.Wavenumber,Spectral_Flux.bb_Flux,'k-','LineWidth',2)
hold on
semilogx(Spectral_Flux.Wavenumber, ...
    Spectral_Flux.bb_Flux+Spectral_Flux.bb_Flux_std,'k:')
semilogx(Spectral_Flux.Wavenumber, ...
    Spectral_Flux.bb_Flux-Spectral_Flux.bb_Flux_std,'k:')
semilogx(Spectral_Flux.Wavenumber, ...
    quantile(Spectral_Flux.bb_Flux_snapshots,0.75,2),'b--')
semilogx(Spectral_Flux.Wavenumber, ...
    quantile(Spectral_Flux.bb_Flux_snapshots,0.25,2),'b--')
semilogx([7 max(Spectral_Flux.Wavenumber)], ...
    mean(Spectral_Flux.bb_Diss_Rate, 'omitnan')*[1 1], 'r-','LineWidth',4)
semilogx([min(Spectral_Flux.Wavenumber) 7], ...
    -mean(Spectral_Flux.bb_Drag_Rate, 'omitnan')*[1 1], 'r-','LineWidth',4)
semilogx([min(Spectral_Flux.Wavenumber) max(Spectral_Flux.Wavenumber)], ...
       [0 0], 'k-','LineWidth',1)
semilogx([7 7], ...
    [-mean(Spectral_Flux.bb_Drag_Rate, 'omitnan') mean(Spectral_Flux.bb_Diss_Rate, 'omitnan')], 'r--')
title('Flux of buoyancy variance')
xlim([min(Spectral_Flux.Wavenumber) max(Spectral_Flux.Wavenumber)])
...
semilogx([min(Spectral_Flux.Wavenumber) max(Spectral_Flux.Wavenumber)], ...
       [0 0], 'k-','LineWidth',1)
semilogx([min(Spectral_Flux.Wavenumber) 7], ...
    -0.5*mean(Spectral_Flux.bb_Drag_Rate, 'omitnan')*[1 1], 'r-','LineWidth',4)
semilogx([7 max(Spectral_Flux.Wavenumber)], ...
    0.5*mean(Spectral_Flux.bb_Diss_Rate, 'omitnan')*[1 1], 'r-','LineWidth',4)
title('Kinetic Energy Flux')
xlim([min(Spectral_Flux.Wavenumber) max(Spectral_Flux.Wavenumber)])
...
% scales
semilogx([7 max(Spectral_Flux.Wavenumber)], ...
    mean(Spectral_Flux.bb_Diss_Rate, 'omitnan')*[1 1], 'color',[0,0,0]+alpha,'LineWidth',4)
hold on
semilogx([min(Spectral_Flux.Wavenumber) 7], ...
    -mean(Spectral_Flux.bb_Drag_Rate, 'omitnan')*[1 1], 'color',[0,0,0]+alpha,'LineWidth',4)
semilogx([min(Spectral_Flux.Wavenumber) max(Spectral_Flux.Wavenumber)], ...
       [0 0], 'k-','LineWidth',1)
semilogx([7 7], ...
    [-mean(Spectral_Flux.bb_Drag_Rate, 'omitnan') mean(Spectral_Flux.bb_Diss_Rate, 'omitnan')],  'color',[0,0,0]+alpha)

% Plot the spectral fluxes
a1=semilogx(Spectral_Flux.Wavenumber,Spectral_Flux.bb_Flux,'k-','LineWidth',4);
a2=semilogx(Spectral_Flux.Wavenumber,Spectral_Flux.bb_Flux_fd,'b-','LineWidth',2);
a3=semilogx(Spectral_Flux.Wavenumber,Spectral_Flux.bb_Flux_fd_flip,'r-','LineWidth',2);
title('Flux of buoyancy variance')
xlim([min(Spectral_Flux.Wavenumber) max(Spectral_Flux.Wavenumber)])
...
% scales
semilogx([7 max(Spectral_Flux.Wavenumber)], ...
    0.5*mean(Spectral_Flux.bb_Diss_Rate, 'omitnan')*[1 1], 'color',[0,0,0]+alpha,'LineWidth',4)
hold on
semilogx([min(Spectral_Flux.Wavenumber) 7], ...
    -0.5*mean(Spectral_Flux.bb_Drag_Rate, 'omitnan')*[1 1], 'color',[0,0,0]+alpha,'LineWidth',4)
semilogx([min(Spectral_Flux.Wavenumber) max(Spectral_Flux.Wavenumber)], ...
       [0 0], 'k-','LineWidth',1)
semilogx([7 7], ...
    0.5*[-mean(Spectral_Flux.bb_Drag_Rate, 'omitnan') mean(Spectral_Flux.bb_Diss_Rate, 'omitnan')],  'color',[0,0,0]+alpha)
