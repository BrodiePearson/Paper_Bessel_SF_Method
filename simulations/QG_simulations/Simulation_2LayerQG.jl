...
n = 1024                  # 2D resolution = n²
timestep_factor = n/128
stepper = "FilteredRK4"  # timestepper
     dt = (2.5e-3)/timestep_factor         # timestep
 nsteps = 2*20000*timestep_factor #20000*timestep_factor/2          # total number of time-steps
 nsubs  = 50 #50             # number of time-steps for plotting (nsteps must be multiple of nsubs)
nothing # hide
...
b = [-1.0, -1.2]           # the density of each layer
...
Fluxes = Diagnostic(MultiLayerQG.fluxes, prob; nsteps)
diags = [E, Fluxes] # A list of Diagnostics types passed to "stepforward!" will  be updated every timestep.
nothing # hide
...
filepath = "./data_2layer"
plotpath = "./plots_2layer"
plotname = "snapshots"
videoname = "multilayerqg_2layer_"*string(n)*"_reference"
filename = filepath*"/multilayerqg_2layer_"*string(n)*"_reference.jld2"
matfilename = filepath*"/multilayerqg_2layer_"*string(n)*"_reference"
nothing # hide
...
function write_matlab_file(writepath, vars, grid, time, diags)
  E, fluxes = diags
  step_path = string(writepath, "_", round(Int, clock.t), ".mat")
  matfile = matopen(step_path, "w")
  write(matfile, "q", vars.q)
  write(matfile, "psi", vars.ψ)
  ...
  write(matfile, "KE", E.data[E.i][1][:])
  write(matfile, "PE", E.data[E.i][2][1])
  write(matfile, "Lat_flux", Fluxes.data[Fluxes.i][1][:])
  write(matfile, "Vert_flux", Fluxes.data[Fluxes.i][2][1])
  close(matfile)
end
...
record(fig, plotpath*"/"*videoname*".mp4", frames, framerate = 18) do j
  if j % (1000 * timestep_factor / nsubs) == 0
    ...
    write_matlab_file(matfilename, vars, grid, clock.t, diags)
    out = Output(prob, filename, (:sol, get_sol), (:u, get_u))
    saveproblem(out)
  end
