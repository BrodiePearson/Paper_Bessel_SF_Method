...
#     nsubs  = round( Int, 50/dt ) #1000            # number of time-steps for intermediate logging/plotting [1000]
 nsteps = 2*2*20000*timestep_factor #20000*timestep_factor/2          # total number of time-steps
...
L = 2π                   # domain size
μ = 5e-2                 # bottom drag
β = 5.0                    # the y-gradient of planetary PV
 
nlayers = 2              # number of layers
f₀ = 1             # Coriolis parameter 
H = [0.2, 0.8]           # the rest depths of each layer
b = [-1.0, -1.0125]           # the buoyancy of each layer
 
U = zeros(nlayers)       # the imposed mean zonal flow in each layer
U[1] = 0.5
U[2] = 0.0
Lᵈ = pi * sqrt( ( b[1]-b[2] )*sum(H))/f₀
nothing # hide
...
axKE = Axis(fig[1, 3], 
            ...
            title = title_KE,
            yscale = log10,
            limits = ((-0.1, 5), (1e-9, 1200)))

axPE = Axis(fig[2, 3], 
            xlabel = "μ t",
            ylabel = "PE",
            yscale = log10,
            limits = ((-0.1, 5), (1e-9, 120)))
